...
Generated ranking from `attention/taf_priority_condorcet.py` after T570 on
2026-07-14:

1. TAF11 - Multi-family falsifier rotation for the TAF11 generator.
   Why: T570 survived fresh-family role recoding but left source-law status
   blocked until multi-family falsifier rotation.
2. TAF8 - Cross-domain shadow-protection theorem target.
   Why: T570 preserved the T541 wait state; internal TAF11 generator hygiene
...
5. TAF4 - Finite-to-continuum finality colimit bridge.
   Why: Still blocked after T570; a finite semantic generator is not continuum
...
| --- | --- | --- | --- |
| TAF11 | open, multi-family falsifier burden selected | Can the fresh-family-surviving TAF11 semantic generator withstand rotating falsifiers across multiple admitted families? | T570 recoded the T568/T569 semantic generator into fresh certificate, attestation, restriction, disagreement, handoff, and audit-refinement vocabulary. It admitted `calibration_chain_role_recoding_family` and `archive_manifest_handoff_role_recoding_family`, rejected replay, missing-square, target-import, optional-payload, and trivial-completion controls, and selected `t571_domain_native_sheaf_transport_multi_family_falsifier_rotation_gate`. |
...
| TAF4 | blocked on source-law survivor | Can native finite finality objects descend toward causal-set or Lorentzian structure? | T526 calibrates target, T528 native preflight incomplete, T532/T534 found no cleared law, T548 narrowed APRD, and T570 supplied only finite semantic-generator stress. TAF4 movement still waits for a real bridge, not a target import. |
...
| TAF8 | open, waiting for cross-domain packet | Can shadow protection transfer across domains without identity-by-construction? | T541 completed the review-only nonidentity witness-packet gate. T570 keeps TAF8 waiting because internal TAF11 generator stress is not a domain-native cross-domain packet under the same spine. |

## next_action

When Joe opens the lane, run the selected multi-family falsifier burden:

Run `t571_domain_native_sheaf_transport_multi_family_falsifier_rotation_gate`
before any claim or canon movement. Rotate falsifiers across the admitted
families and try to break the role-level generator, while preserving the same
target import, cross-repo truth, Observerse replay, APRD replay, TAF8
...
data-bearing packet exists. TAF3 remains blocked until TAF11 multi-family
falsifier rotation clears or abandons the semantic generator route, or TAF12 supplies a real
