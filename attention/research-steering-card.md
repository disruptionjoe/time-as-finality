...
Generated ranking from `attention/taf_priority_condorcet.py` after T556 on
2026-07-13:

1. TAF11 - North Star source-law reassessment beyond the finite-generator route.
   Why: T556 parked the absorbed Observerse protocol-stack route as audit
   translation residue and selected T557 fresh-family preflight.
2. TAF8 - Cross-domain shadow-protection theorem target.
   Why: T556 preserved the T541 wait state; internal TAF11 route reset is not a
...
5. TAF4 - Finite-to-continuum finality colimit bridge.
   Why: Still blocked after T556; parking the absorbed Observerse route does
   not supply a finite-to-continuum bridge.
...
| --- | --- | --- | --- |
| TAF11 | open, route reset to fresh-family preflight | What fresh source-law family can be declared with source variables, mature absorbers, and falsifiers before target reading? | T556 consumed T555 absorber completion, parked the T550-T555 Observerse protocol-stack route as audit translation residue, rejected more bounded Observerse fixtures as source-law evidence, and selected `t557_taf11_fresh_source_law_family_preflight_gate`. |
...
| TAF4 | blocked on fresh TAF11 family | Can native finite finality objects descend toward causal-set or Lorentzian structure? | T526 calibrates target, T528 native preflight incomplete, T532/T534 found no cleared law, T548 narrowed APRD, and T556 parked the Observerse route. TAF4 movement still waits for a real generalization bridge. |
...
| TAF8 | open, waiting for domain-native packet | Can shadow protection transfer across domains without identity-by-construction? | T541 completed the review-only nonidentity witness-packet gate. T556 keeps TAF8 waiting because internal TAF11 route reset is not a domain-native cross-domain packet under the same spine. |

## next_action

When Joe opens the lane, run the TAF11 fresh-family preflight:

Run `t557_taf11_fresh_source_law_family_preflight_gate`. Name a new TAF11
source-law family, declare its source variables, predeclare mature absorbers and
falsifiers, and reject if the packet reuses the absorbed Observerse
protocol-stack route as evidence, imports cross-repo truth, executes TAF8, or
moves TAF4/source-law status directly.
...
exists. TAF3 remains blocked until TAF11 supplies a fresh declared family with
a real survivor or TAF12 supplies a real noncompletion target.
