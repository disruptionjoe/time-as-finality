...
def run_t588_analysis() -> T588Result:
    ...
    return T588Result(
        ...
        next_work=(
            "The A-versus-C discriminator is the live question and it concerns "
            "reconciliation at contact, not comparators on a frozen event "
            "system. Note that contract B, had it survived, would have derived "
            "a global tick from the ledger count rather than positing one; its "
            "refutation therefore closes the cheapest route to a preferred "
            "frame in this programme."
        ),
    )
