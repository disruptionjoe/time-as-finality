# T323 Results: Hard Lowcover

## Aggregate Checks

| Metric | Value |
| --- | --- |
| `description` | `Hard gate: T126+band and parent cover hub <= 2/7.` |
| `total_weight` | `185` |
| `tail_weight` | `10` |
| `tail_probability` | `{'fraction': '2/37', 'float': 0.05405405405405406}` |
| `parentcap_probability` | `{'fraction': '66/185', 'float': 0.3567567567567568}` |
| `selected_probability` | `{'fraction': '1/185', 'float': 0.005405405405405406}` |
| `lift_vs_uniform` | `{'fraction': '72576/37', 'float': 1961.5135135135135}` |
| `uses_tail_label` | `False` |
| `empirical_target_equivalent` | `False` |
| `uses_deletion_t252_count` | `False` |

- Verdict: `hard_lowcover_tail_probability_recorded`

## Strongest Claim

The `hard_lowcover` candidate assigns the 10-case tail probability 2/37.
...
T323 fails if the `hard_lowcover` weights are changed without renaming the measure.
...
Do not update the claim ledger from T323 alone.
