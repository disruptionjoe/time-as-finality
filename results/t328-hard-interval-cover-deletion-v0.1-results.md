# T328 Results: Hard Interval Cover Deletion

## Aggregate Checks

| Metric | Value |
| --- | --- |
| `description` | `Hard gate: interval<=3, low cover, and deletion-band stability.` |
| `total_weight` | `10` |
| `tail_weight` | `10` |
...
- Verdict: `hard_interval_cover_deletion_tail_probability_recorded`

## Strongest Claim

The `hard_interval_cover_deletion` candidate assigns the 10-case tail probability 1.
...
T328 fails if the `hard_interval_cover_deletion` weights are changed without renaming the measure.
...
Do not update the claim ledger from T328 alone.
