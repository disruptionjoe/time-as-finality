# T345 Results: Soft Coverhub Inverse

## Aggregate Checks

| Metric | Value |
| --- | --- |
| `description` | `Soft score giving an 8x bonus to cover hub <= 1/4 inside the band.` |
| `total_weight` | `364175` |
| `tail_weight` | `80` |
| `tail_probability` | `{'fraction': '16/72835', 'float': 0.00021967460698839843}` |
| `parentcap_probability` | `{'fraction': '528/364175', 'float': 0.0014498524061234298}` |
| `selected_probability` | `{'fraction': '8/364175', 'float': 2.1967460698839844e-05}` |
| `lift_vs_uniform` | `{'fraction': '82944/10405', 'float': 7.971552138395002}` |
| `uses_tail_label` | `False` |
| `empirical_target_equivalent` | `False` |
| `uses_deletion_t252_count` | `False` |

- Verdict: `soft_coverhub_inverse_tail_probability_recorded`

## Strongest Claim

The `soft_coverhub_inverse` candidate assigns the 10-case tail probability 16/72835.

## What This Improved

This records an exact finite probability for a declared candidate weighting rule.

## What This Weakened Or Falsified

The candidate is weakened as an S1-relevant measure unless it is both non-tautological and independently derivable.

## Falsification Condition

T345 fails if the `soft_coverhub_inverse` weights are changed without renaming the measure.

## S1 Update

S1 remains requires_added_assumption for the finite ordinal route.

## Claim Ledger Update

Do not update the claim ledger from T345 alone.

## Open Blocker

A finite weighting rule is not a physical measure until derived from finality-domain dynamics.

## Suggested Next

Compare this probability against the hard-gate and soft-score rankings, then derive or reject the leading candidate.

## Not Claimed

These finite measure stress tests do not define a physical measure, estimate dimension, prove faithful embedding, validate random sprinkling, derive metric structure, establish a continuum limit, or settle S1.
